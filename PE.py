...
for y in range(1,pe_size+1):
    for x in range(1,pe_size+1):
        if(y&(pe_size/4)==1):#最上面那排要有雙TSV
            if(x%(pe_size/4)/4==1 or x%(pe_size/4)/4==0):
                ...
                print("workstate[0],clk,OSIS[OSISnum["+str(counter_input)+"]],OSWS[OSWSnum["+str(0)+"]],tsv_out["+str(x)+"]["+str(y)+"],rst,42'd"+str(x)+",42'd"+str(y))
                ...
                print("output_left["+str(x)+"]["+str(y)+"],output_right["+str(x)+"]["+str(y)+"],output_up["+str(x)+"]["+str(y)+"],output_down["+str(x)+"]["+str(y)+"],",end='')
                print("workstate[0],clk,OSWS[OSWSnum["+str(counter_weight)+"]],tsv_out["+str(x)+"]["+str(y)+"],rst,42'd"+str(x)+",42'd"+str(y)+",size,kernelsize,result["+str(x)+"]["+str(y)+"]);")
            counter_weight = counter_weight + 1
            counter_input = counter_input + 1
        elif(x%(pe_size/4)/4==0 or y%(pe_size/4)==0):#右邊跟下面只會有weight的輸入
            ...
            print(",output_left["+str(x)+"]["+str(y)+"],output_right["+str(x)+"]["+str(y)+"],output_up["+str(x)+"]["+str(y)+"],output_down["+str(x)+"]["+str(y)+"],",end='\t')
            print("workstate[0],clk,OSIS[OSISnum["+str(counter_input)+"]],tsv_out["+str(x)+"]["+str(y)+"],rst,42'd"+str(x)+",42'd"+str(y)+",size,kernelsize,result["+str(x)+"]["+str(y)+"]);")
            counter_input = counter_input + 1
        else:
            ...
            print("output_left["+str(x)+"]["+str(y)+"],output_right["+str(x)+"]["+str(y)+"],output_up["+str(x)+"]["+str(y)+"],output_down["+str(x)+"]["+str(y)+"],",end='\t')
            print("workstate[0],clk,rst,42'd"+str(x)+",42'd"+str(y)+",size,kernelsize,result["+str(x)+"]["+str(y)+"]);")
